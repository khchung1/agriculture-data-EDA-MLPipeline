...
- The regression_model() and classifier_model() method take tuple as argument which is
use in one of the steps in Pipeline method from sklearn. Hence to change the ML model or configure its hyperparameters, edit the elements in the tuple accordingly.
...
- Takes in cleaned and processed data and run two methods (ML models):
    - regression_model(): 
    ...
    - best_regression_model()
        - aims to find the best regression model out of 3 models (Linear Regression, Decision Tree, and Random Forest)
        - uses GridSearchCV to find the best hyperparameters for each model based on R-squared score.
        - evaluates each model using Mean Squared Error and R-squared.
        - selects the model with the highest R-squared score as the best model.
    ...
    - best_classification_model()
        ...
        - defines 3 classification models, Logistic Regression, SVM and Random Forest with their hyperparameter grids for tuning
        - get best model by evaluating model performance using accuracy score and a classification report
...
3 categorical models are selected with different alorithim approaches:
